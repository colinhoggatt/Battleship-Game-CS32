...
inline
bool compareSensorPtr(const Sensor* lhs, const Sensor* rhs) //DONE
{
...
void insertion_sort(vector<Sensor>& s, bool comp(const Sensor&, const Sensor&))
{
    ...
    //new
    //using augmented txtbook code
    for (int unsorted = 1; unsorted < s.size(); unsorted++) {
        int loc = unsorted;
        while (loc > 0 and comp(s[loc], s[loc - 1])) {
            swap(s[loc - 1], s[loc]);
            loc--;
        }
    }
...
void sortUsingPtrs(vector<Sensor>& sensors, bool comp(const Sensor*, const Sensor*))
{
    ...
    {
        sensorPointers.push_back(&auxSensors.at(i));
    }
    ...
    {
        //set auxSensor value to sorted dereferenced sensorPointer at i;
        sensors.at(i) = *sensorPointers.at(i);
    }
