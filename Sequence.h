...
template <typename ItemType>
ItemType subsequence (const ItemType& seq1, const Sequence<ItemType>& seq2);
...
template <typename ItemType>
void interleave(const ItemType& seq1, const Sequence<ItemType>& seq2, ItemType& result);
// Set result to a Sequence that interleaves seq1 and seq2.


//member function implementations (copied from .cpp)
// Sequence.cpp
...
template <typename ItemType>
int subsequence(const Sequence<ItemType>& seq1, const Sequence<ItemType>& seq2) //Is this still fine even though it's supposed to return an int?
{
    if (seq2.empty())
        return -1;
